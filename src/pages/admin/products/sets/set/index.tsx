...
export const Set: React.FC<SetDetailProps> = ({ set, onBack }) => {
    ...
    const [brandId, setBrandId] = useState(set.brand.id);
    const [selectedBrand] = useState(set.brand.name)
    ...
    const handleUpdate = async () => {
        ...
        if (!setName || !setCode || !description || !brandId) {
            setError(
                'Set Name, Set Code, Description, and Brand are required.',
            );
            setIsUpdating(false);
            return;
        }
